...
template<typename T>
T NTTCalculator<T>::initModRootOrd(const std::size_t bound)
{
    if(bound < MOD_INT)
    {
        MOD<T> = MOD_INT;
...
template<typename T>
void NTTCalculator<T>::multiply(const FormalPowerSeries<Zp<T>>& x,
                                ...
                                FormalPowerSeries<Zp<T>>& result)
{
    ...
    for(std::size_t i = 0 ; i < adjustedLength; ++i )
        A[i] = ((((__int128) A[i] * B[i]) % MOD<T>) * inversionLength) % MOD<T>;
...
template<typename T>
void NTTCalculator<T>::multiply(const std::vector<T>& x,
