...
template <typename T>
void FormalPowerSeries<T>::releaseMemory()
{
  FormalPowerSeries<T>().swap(*this);
}

template <typename U>
FormalPowerSeries<U> operator*(const FormalPowerSeries<U>& a,
                               const FormalPowerSeries<U>& b)
{
  FormalPowerSeries<U> result;
  if constexpr(FORMAL_SERIES_MULT_POLICY == FFT)
    FFTCalculator<U>::multiplyNoOverlow(a, b, result);
  else if(FORMAL_SERIES_MULT_POLICY == NTT)
  {
    if constexpr(is_Zp<U>::value)
      NTTCalculator<typename U::underlying_type>::multiply(a, b, result);
    /* ! */
  }
  else
  {
    assert(a.size() > 0 && b.size() > 0);
    result.resize(a.size() + b.size() - 1, U(0));
...
FormalPowerSeries<T> reciprocal(const FormalPowerSeries<T>& input)
{
  ...
  FormalPowerSeries<T> b(1, T::getInversion(input[0]));
...
//      inputPrefix.resize(2 * i + 2, T(1));
  ...
  if constexpr (CHECK_RECIPROCAL)
  {
      ...
      bool correct = true;
      correct &= test[0] == T(1);
      correct &= std::all_of(test.begin() + 1,
                             test.end(),
...
FormalPowerSeries<T> log(const FormalPowerSeries<T>& input)
{
  ...
  for(std::size_t i = 1 ; i < result.size(); ++i)
      result[i] = T::getInversion(i) * shiftedResult[i - 1];
  assert(input.size() == result.size());
  return result;
}
...
FormalPowerSeries<T> exp(const FormalPowerSeries<T>& input)
{
    ...
    std::function<void(std::size_t, std::size_t)> compute
        ...
        if(l < r)
        {
            ...
            for(std::size_t i = m + 1; i <= r; ++i)
              result[i] += T::getInversion(i) * H[i - l];
  ...
  else
  {
    ...
    for (std::size_t i = 1; i < input.size(); i *= 2) {
        ...
        assert(std::all_of(partialResult.begin(),
                           partialResult.begin() + i,
...
template class
FormalPowerSeries<Zp<Int>>;

template
FormalPowerSeries<Zp<Int>> exp<Zp<Int>, DIVIDE_AND_CONQUER>(const FormalPowerSeries<Zp<Int>>& input);
template
FormalPowerSeries<Zp<Int>> exp<Zp<Int>, NEWTONS_ITERATIVE_METHOD>(const FormalPowerSeries<Zp<Int>>& input);
